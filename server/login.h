...
public:
  ...
  void insert(std::string uid, std::string pass, int isGamer,
  ...
  std::pair<status, Basic::Gamer> getGamer(std::string uid, std::string pass) {
    Basic::Gamer g;
  ...
  std::pair<status, Basic::Admin> getAdmin(std::string uid, std::string pass) {
    Basic::Admin g;
    status test = S_NOERROR;
    int count = 0;
    db << "SELECT COUNT(*) FROM user where uid=?" << uid >> count;
