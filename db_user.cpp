...
UserDB::UserDB(const char* path) {
    ...
    qry.prepare(
        ...
        ");");
    if(!qry.exec()) {
        qDebug() << QString::fromStdString(qry.lastError().driverText().toStdString());
...
BaseUser UserDB::loginUser(const LoginRequest &info) {
    QSqlQuery qry;
    BaseUser user;
    QString qryStr = "SELECT uname, rname, level, exp FROM user "
                     "WHERE uname = '%1' AND password = '%2' AND utype = %3 LIMIT 1";
    qry.prepare(qryStr.arg(info.userName).arg(info.userPassword).arg(info.userType));
...
bool UserDB::updateUser(const BaseUser* user) {
    QSqlQuery qry;
    QString qryStr =
        "UPDATE user SET level=%1,exp=%2 WHERE uname='%4';";
    qry.prepare(qryStr.arg(user->levelPassed).arg(user->experience).arg(user->userName));
    return qry.exec();
}
...
void UserDB::searchGamers(QVector<QVector<QString>>&model, QString query) {
    QString qryStr = "SELECT uname,rname,exp,level FROM user WHERE utype=1 AND %1";
    QSqlQuery qry;
...
void UserDB::searchAdmins(QVector<QVector<QString>>&model, QString query) {
    QString qryStr = "SELECT uname,rname,exp,level FROM user WHERE utype=0 AND %1";
    QSqlQuery qry;
    qry.prepare(qryStr.arg(query));
    if (qry.exec()) {
        while(qry.next()) {
